 ...
 * 저장소에서 찾은 결정 후보.
 *
 * 처음에는 최근 커밋·PR 제목을 그대로 내밀었다. 그러자 "fix(체크인): 셀카
 * 완료 상태를 서버 기준으로 전환" 같은 잔버그 제목이 후보가 됐다 — 최근
 * 것만 가져오니 판단이 없었던 변경이 말할 만한 결정처럼 서 있었다.
 *
 * 그래서 모델이 커밋·PR 전체를 읽고 **주제로 묶는다**. 잔수정은 주제가 되지
 * 못하고, 흩어진 커밋 여럿이 하나의 결정으로 모인다. 결과는 근거에 붙여
 * 캐시하므로 모델 호출은 저장소마다 한 번이다.
 *
 * 분류가 안 되는 경우(근거 없음·모델 실패)에는 제목 나열로 물러난다 —
 * 후보가 나쁜 것과 결정을 못 쓰는 것은 다른 일이다.
 */
export interface PortfolioDecisionCandidateDto {
  /** 고르면 질문에 붙는 한 줄. 지원자가 "아, 그거" 할 수 있어야 한다. */
  topic: string;
  /** 무슨 일이 있었는지 한 줄. 제목 나열로 물러난 경우 본문 첫 줄이다. */
  summary: string | null;
  /** 이 주제를 뒷받침하는 커밋·PR 제목 그대로. 제목 나열에서는 빈 배열. */
  evidence: string[];
  /** analysis = 모델이 묶은 주제. commit·pullRequest = 제목 나열 폴백. */
  source: "analysis" | "commit" | "pullRequest";
}
