...
사용자가 저장소 카드를 클릭하면 선택 상태를 토글한다. 프론트 퍼블리싱과
mock 단계에서는 첫 저장소 ID를 경로에 유지하고 전체 선택 ID는 query string으로
전달한다. 실제 다중 저장소 분석은 `repositoryIds` API 계약과 백엔드 저장 모델을
합의한 뒤 연결하며, 그 전까지 HTTP adapter는 단일 저장소 계약을 유지한다.
...
- 현재 REST API 연동 기준의 대표 `repositoryId`
- `prompt`
