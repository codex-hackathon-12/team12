...
처음에는 최근 커밋·PR 제목을 그대로 돌려줬다. 그러자 잔버그 수정 제목이
후보가 됐다 — 최근 것만 가져오니 판단이 없었던 변경이 말할 만한 결정처럼
서 있었다. 지금은 모델이 본인 커밋·PR 전체를 읽고 **주제로 묶는다.**

분류 결과는 생성 근거에 붙여 캐시하므로 모델 호출은 저장소마다 한 번이다.
크레딧을 쓰지 않는다.
...
```json
{
  "data": [
    {
      "topic": "체크인 상태의 기준을 서버로 옮김",
      "summary": "셀카 완료 여부를 클라이언트가 들고 있다가 서버 기준으로 바꿨다.",
      "evidence": ["fix(체크인): 셀카 완료 상태를 서버 기준으로 전환"],
      "source": "analysis"
    }
  ]
}
...
`source`가 `analysis`면 모델이 묶은 주제이고, `commit`·`pullRequest`면 분류가
안 되어(근거 없음·모델 실패) 제목 나열로 물러난 것이다. 폴백에서는 `summary`에
본문 첫 줄이 오고 `evidence`는 빈 배열이다. 병합·잡일 커밋은 어느 쪽에서도
걸러진다.
