...
const instructions = [
  ...
  "질문은 이미 비어 있는 자리에 대해서만 만드세요. 방금 채운 항목을 다시 묻지 마세요. role은 예외로, 기여자가 여럿이거나 ownContributionUnverifiable이 true여서 담당 범위가 실제로 불분명할 때만 묻습니다.",
  "질문은 그 저장소에서 실제로 본 것을 짚어 구체적으로 묻습니다. 무엇을 묻는지가 질문 문장 안에서 완결돼야 하고, 지원자가 한두 문장으로 답할 수 있어야 합니다. 예를 들어 재시도 처리를 추가한 커밋이 있는데 그 이유가 어디에도 없으면 '재시도 처리를 넣기 전에는 어떤 문제가 있었나요?'라고 묻습니다.",
  "'성과를 알려주세요', '어려웠던 점은 무엇인가요'처럼 어느 프로젝트에나 붙는 질문은 쓰지 마세요. 무엇을 묻는지 모르면 사람은 답하지 못하거나 아무 말이나 적게 되고, 그러면 되묻는 의미가 없습니다.",
  ...
  "repositoryName에는 질문 대상 저장소의 name을 정확히 그대로 적고, field에는 그 답이 채울 자리를 적습니다. 한 프로젝트에 두 개까지, 전체 다섯 개 안쪽이면 충분합니다. 물을 것이 없으면 빈 배열을 반환하세요.",
