 ...
 * 목적지의 이름.
 *
 * 같은 화면을 가리키는 링크가 자리마다 다른 말을 쓰고 있었다. `/repositories`
 * 하나만 해도 포트폴리오 만들기·새로 만들기·첫 포트폴리오 만들기·내 포트폴리오
 * 만들기·다시 만들기로 다섯 가지였다. 사용자는 그것들이 같은 곳인지 알 수 없다.
 *
 * 목적지의 이름은 여기서 한 번만 정하고, 문장 안에서 필요한 꾸밈은 각 화면이
 * 붙인다. 이름 자체는 어디서나 같아야 한다.
 */
export const LABEL = {
  dashboard: "대시보드",
  portfolios: "내 포트폴리오",
...
/**
 * 모바일 탭처럼 폭이 모자란 자리에서만 쓰는 축약.
 *
 * 예전에는 셸이 데스크톱 라벨을 정의한 뒤 모바일에서 문자열을 비교해 다시
 * 썼다. 라벨을 고치면 축약이 조용히 원래 이름으로 돌아갔다.
 */
export const SHORT_LABEL: Partial<Record<keyof typeof LABEL, string>> = {
  portfolios: "내 작업",
  create: "만들기",
};

/**
 * 결제와 크레딧이 아직 체험이라는 사실.
 *
 * 같은 사실을 다섯 가지 문장으로 말하고 있었다. 차감 없음·잔액 변동 없음·
 * MVP에서는 차감되지 않아요·mock 흐름이므로 변화가 없습니다·실제 승인이나
 * 지급은 발생하지 않았습니다. 읽는 사람은 이것들이 같은 말인지 확신할 수 없고,
 * 확신이 필요한 화면이 하필 결제 직전이다.
 */
export const MOCK_CHIP = "체험 · 차감 없음";
export const MOCK_NOTE = "아직 체험 단계라 크레딧이 실제로 차감되지 않아요.";
